        ...
        try:
            ...
            for setting_key in ["pr_code_suggestions_prompt", "pr_code_suggestions_prompt_not_decoupled"]:
                ...
                verification_prompt = (
                    ...
                    "2. DISCOVER: Identify any ADDITIONAL/NEW bugs, security issues, or performance problems that the previous agent missed. YOU MUST STRICTLY LIMIT your discoveries to the explicit guidelines provided. Do NOT use general knowledge or opinions. Do NOT change or complain about business logic.\n\n"
                    "STRICT QUALITY STANDARDS (You MUST drop any suggestion that violates these):\n"
                    "- STRICT DOMAIN LIMIT: When discovering new issues, you must only report: hardcoded secrets/credentials, SQL injections, unhandled exceptions, N+1 queries, and missing dbt macros. Do not report anything outside this strict checklist.\n"
                    "- SECURITY CRITICAL: If code contains hardcoded secrets, passwords, API keys, or credentials (e.g. STRIPE_SECRET_KEY = 'sk_live_...'), you MUST include a suggestion to move them to environment variables (e.g. os.getenv('KEY_NAME')). These are always valid, high-priority findings. Do NOT drop them.\n"
